...
| **I34** | Compliance "risk classification" thresholds | Spec 7.5 lists a `risk classification` column per transaction type but gives **no thresholds** | Classify by the type's fraud rate %: `>= 5%` → **High**, `>= 1%` → **Medium**, else **Low**. Overridable via `COMPLIANCE_RISK_HIGH_PCT` / `COMPLIANCE_RISK_MEDIUM_PCT`. **Needs owner sign-off.** | OPEN |
| **I35** | Compliance window | Spec 7.5: "168-step / 7-day window" | `WHERE step BETWEEN 1 AND 168` (`COMPLIANCE_WINDOW_STEPS`). The synthetic dataset is exactly 168 steps, so this is the whole history; kept explicit for a larger feed. | ADOPTED |
| **I36** | Dormancy severity boundaries | Spec 7.6: base "> 72 steps"; "72–120 = Dormant, >120 = Severely Dormant" | Base condition is strict `steps_inactive > 72` (72 itself is **not** dormant). `73–120` → **Dormant**, `> 120` → **Severely Dormant**. `steps_inactive = max(step) over all data − customer's last step`. | ADOPTED |
| **I37** | `customer_fraud_summary` scope + `dormancy` merchant exclusion | Spec 7.6: "account begins with C … merchants beginning with M are excluded"; 7.5/7.6 customer grain not fully specified | Both modes filter `nameOrig LIKE 'C%'` (customer-originated legs only; `nameDest` merchant legs never appear as `nameOrig` in the PaySim schema anyway). `customer_fraud_summary` columns: `customerId, total_transactions, total_amount, confirmed_fraud_count, fraud_rate_pct`. Parquet, overwrite. | ADOPTED |
| **I38** | Compliance / dormancy CSV exports | Spec 7.6 asks for `/finsight/exports/dormancy_report.csv` (a single file); 7.5 export path not given | `dormancy` writes Parquet **and** one CSV at exactly `/finsight/exports/dormancy_report.csv` (coalesce(1) + rename the `part-*` file). `compliance` mirrors this to `/finsight/exports/compliance_summary.csv` for Alteryx/Power BI (I26 pattern). `customer_summary` is Parquet only (spec 7.6 names no CSV). | ADOPTED |
| **I39** | `customers` document identity | Spec 10: "preserve customerId exactly … the join key" | `mongoimport` keeps every source field verbatim; Mongo adds its own ObjectId `_id`. `customerId` is kept as the source **string** and enforced by a **unique** index (`ux_customerId`), not promoted to `_id`. Re-import is `--mode upsert --upsertFields customerId` (idempotent). | ADOPTED |
| **I40** | Required compound index | Spec 10: "compound index using customerId and segment" | `{ customerId: 1, segment: 1 }` (`ix_customerId_segment`). Also created: unique `{ customerId: 1 }` (join key) and `{ segment: 1 }` (segment dashboards / count validation). | ADOPTED |
| **I41** | Segment-count validation | Spec 10 lists the 5 segments but gives no target counts | Validate structurally, not against fixed numbers: all 5 of `Premium, Standard, Basic, Private Banking, Student` present with ≥1 customer, no segment value outside that set, per-segment counts sum to the collection total (no NULL/missing segment). Actual load: Standard 4497, Basic 2464, Premium 1528, Student 983, Private Banking 528 = 10 000. | ADOPTED |
| **I42** | Customer dataset location | Not specified | `mongodb/import_customers.sh` searches `CUSTOMERS_JSON` (`.env`), `data/raw/noveacrest_customers.json`, then `Bigdata Data set file/src-data/…`. The file (~5.5 MB JSON array) is **git-ignored** — not committed. Import streams it over stdin into `mongoimport` inside `finsight-mongodb` (no host mongo tools needed). | ADOPTED |
...
| G3 | MongoDB | `mongoimport` command referenced but absent | RESOLVED Phase 10: `mongodb/import_customers.sh` — `mongoimport --uri … --collection customers --jsonArray --mode upsert --upsertFields customerId`, streamed over stdin into `finsight-mongodb`. | 10 ✓ |
...
| G15 | MongoDB | "expected distribution" for segment validation not numerically given | RESOLVED Phase 10 (I41): structural validation — all 5 segments present, no unexpected values, counts sum to the collection total. `mongodb/validation.js` + `mongodb/segments.py`. | 10 ✓ |
